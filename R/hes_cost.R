...
hes_cost <-  function(
  ...
  need_opcs_cost <- costed_hes[stringr::str_sub(icd_code, 1, 1) == "C" | stringr::str_sub(icd_code, 1, 3) == "N18"]
  dont_need_opcs_cost <- costed_hes[stringr::str_sub(icd_code, 1, 1) != "C" & stringr::str_sub(icd_code, 1, 3) != "N18"]
  
  # Create a temporary file with just the columns we want.
  temp <- need_opcs_cost[, subset_diags, with = F]
  ...
  d4_costs <- paste0("d4_cost_", 1:24)
  temp[, OPCS_cost := rowSums(temp[, d4_costs, with = F], na.rm = T)]
  ...
  # all opcs costs are for 2016
  
  hes[Year == 2010, `:=`(cost_2016 = cost * 1.092519, Excess_costs_2016 = Excess_costs * 1.092519)]
  hes[Year == 2011, `:=`(cost_2016 = cost * 1.070088, Excess_costs_2016 = Excess_costs * 1.070088)]
  hes[Year == 2012, `:=`(cost_2016 = cost * 1.05221, Excess_costs_2016 = Excess_costs * 1.05221)]
  hes[Year == 2013, `:=`(cost_2016 = cost * 1.04062, Excess_costs_2016 = Excess_costs * 1.04062)]
  hes[Year == 2014, `:=`(cost_2016 = cost * 1.031389, Excess_costs_2016 = Excess_costs * 1.031389)]
  hes[Year == 2015, `:=`(cost_2016 = cost * 1.017845, Excess_costs_2016 = Excess_costs * 1.017845)]
